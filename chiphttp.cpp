...
void ChipHttp::HexChar(unsigned char c, unsigned char &hex1, unsigned char &hex2) {
    hex1 = c / 16;
    hex2 = c % 16;
    hex1 += hex1 <= 9 ? '0' : 'a' - 10;
    hex2 += hex2 <= 9 ? '0' : 'a' - 10;
...
string ChipHttp::URLEncode(string &value) {
	const char *str = value.c_str();
	vector<char> v(value.size());
	v.clear();
